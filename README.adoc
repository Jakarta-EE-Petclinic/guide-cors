...
=== Enabling a simple CORS configuration
Configure the server to allow the `/configurations/simple` endpoint to accept a [hotspot=simple-config file=0]`simple` CORS request.
...
|[hotspot=domain-simple file=0]`domain` | The endpoint to be configured for CORS requests. The value is set to `/configurations/simple`.
|[hotspot=allowedOrigins-simple file=0]`allowedOrigins` | Origins that are allowed to access the endpoint. The value is set to `openliberty.io`.
|[hotspot=allowedMethods-simple file=0]`allowedMethods` | HTTP methods that a client is allowed to use when it makes requests to the endpoint. The value is set to `GET`.
|[hotspot=allowCredentials-simple file=0]`allowCredentials` | A boolean that indicates whether the user credentials can be included in the request. The value is set to `true`.
|[hotspot=exposeHeaders-simple file=0]`exposeHeaders` | Headers that are safe to expose to clients. The value is set to `MyHeader`.
|===
...
Now we will test the simple CORS configuration that you added. Add the [hotspot=testSimpleCorsRequest file=1]`testSimpleCorsRequest` method to the `TestCors` class.
...
TestCors.java
[source, java, linenums, role='code_column hide_tags=comment,testPreflightCorsRequest']
----
include::finish/src/test/java/it/io/openliberty/guides/cors/TestCors.java[]
----
...
The [hotspot=testSimpleCorsRequest file=1]`testSimpleCorsRequest` test passes and prints the response headers with their values from the endpoint. The
...
Configure the server to allow the `/configurations/preflight` endpoint to accept a [hotspot=preflight-config file=0]`preflight` CORS request.
...
|===
| *Configuration Attribute* | *Value*
| [hotspot=domain-preflight file=0]`domain`|The value is set to `/configurations/preflight` because the `domain` is a different endpoint.
| [hotspot=allowedOrigins-preflight file=0]`allowedOrigins`| Origins that are allowed to access the endpoint. The value is set to an asterisk (*) to allow requests from all origins.
| [hotspot=allowedMethods-preflight file=0]`allowedMethods`| HTTP methods that a client is allowed to use when it makes requests to the endpoint. The value is set to `OPTIONS, DELETE`.
| [hotspot=allowCredentials-preflight file=0]`allowCredentials`| A boolean that indicates whether the user credentials can be included in the request. The value is set to `true`.
|===

The follow attributes were added:

* [hotspot=maxAge file=0]`maxAge`: The number of seconds that a client can cache a response to a preflight request. Set the value to `10`.
* [hotspot=allowedHeaders-preflight file=0]`allowedHeaders`: Headers that a client can use in requests. Set the value to `MyOwnHeader1, MyOwnHeader2`.
...
|===
| *Response Header* | *Response Value*
| Access-Control-Allow-Max-Age | The expected value is `10`. Indicates that the preflight request can be cached within `10` seconds.
...
The [hotspot=testPreflightCorsRequest file=1]`testPreflightCorsRequest` test passes and prints the response headers with their values from the endpoint.
